...
// ReadFile reads the file named by filename from the absfs.FileSystem fs and
// returns the contents. A successful call returns err == nil, not err == EOF.
...
func ReadFile(fs absfs.FileSystem, filename string) ([]byte, error) {
	...
	defer f.Close()
	// It's a good but not certain bet that FileInfo will tell us exactly how much
	// to read, so let's try it but be prepared for the answer to be wrong.
	var n int64
...
// WriteFile writes data to a file named by filename in the absfs.FileSystem fs.
